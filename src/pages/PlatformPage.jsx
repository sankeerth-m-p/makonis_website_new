...
import { platformPageContent } from '../config/homeContent.js'

function PlatformPage({ platformKey }) {
  const { navigate } = useAppRoute()
  const content = platformPageContent[platformKey]
        ...
        <div className="container-default">
          <div className="grid gap-6 md:grid-cols-3">
            {content.highlights.map((highlight) => (
              <div
                key={highlight}
                className="rounded-2xl border border-black/10 bg-white p-6 shadow-[0_10px_30px_rgba(0,0,0,0.04)]"
              >
                <p className="text-sm font-semibold uppercase tracking-[0.16em] text-makonis-blue">
                  Capability
                </p>
